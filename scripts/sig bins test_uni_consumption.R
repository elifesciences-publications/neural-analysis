load("C:/Users/Kevin Caref/Google Drive/RScripts/Functions/KC.sigbins_uni_cons.Rfunc")
...
presigbins = KC.sigbins_uni_cons.Rfunc(path = paste(filepath, "unilateral ctap/", sep = ""), startt, endt, event, side)
 
postsigbins = KC.sigbins_uni_cons.Rfunc(path = paste(filepath, "unilateral ctap/", sep = ""), startt=2720, endt=4720, event, side)
...
mtext("% Excited Neurons", side = 2, line = 3, cex = 2)
...
########################################################################################################
prepctexbins = sapply(seq(1:ncol(sigpre)), function(x) length(which(sigpre[,x] == T))/8)   #divide here by number of bins in 3 s
postpctexbins = sapply(seq(1:ncol(sigpost)), function(x) length(which(sigpost[,x] == T))/8)   #divide here by number of bins in 3 s
