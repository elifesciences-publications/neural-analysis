...
expresigbins = KC.sigbins.Rfunc(path = paste(filepath, "restricted ctap/", sep = ""), startt, endt, event)#[,-torem]
 
expostsigbins = KC.sigbins.Rfunc(path = paste(filepath, "restricted ctap/", sep = ""), startt=2720, endt=4720, event)#[,-torem]
...
inpresigbins = KC.sigbins_inhib.Rfunc(path = paste(filepath, "restricted ctap/", sep = ""), startt, endt, event)#[,-torem]
 
inpostsigbins = KC.sigbins_inhib.Rfunc(path = paste(filepath, "restricted ctap/", sep = ""), startt=2720, endt=4720, event)#[,-torem]
...
########################################################################################################
prepctexbins = sapply(seq(1:ncol(exsigpre)), function(x) length(which(exsigpre[,x] == T))/nrow(exsigpre))   #divide here by number of bins in 3 s
postpctexbins = sapply(seq(1:ncol(exsigpost)), function(x) length(which(exsigpost[,x] == T))/nrow(exsigpre))   #divide here by number of bins in 3 s

prepctinbins = sapply(seq(1:ncol(insigpre)), function(x) length(which(insigpre[,x] == T))/nrow(exsigpre))   #divide here by number of bins in 3 s
postpctinbins = sapply(seq(1:ncol(insigpost)), function(x) length(which(insigpost[,x] == T))/nrow(exsigpre))   #divide here by number of bins in 3 s
