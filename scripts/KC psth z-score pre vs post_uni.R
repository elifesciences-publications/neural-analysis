...
ipsipre = unineuralhist.rFunc(path = paste(filepath, "unilateral ctap/", sep = ""), startt,
 endt, binw, psthmin, psthmax, event, cueexonly, side)
 
ipsipost = unineuralhist.rFunc(path = paste(filepath, "unilateral ctap/", sep = ""), startt=2720,
 endt=4720, binw, psthmin, psthmax, event, cueexonly=F, side="ipsi") 
 
 

cueexneurons = ipsipre[[2]]
 
pre = ipsipre[[1]]
post = ipsipost[,cueexneurons] 
...
axis(2, at = c(seq(0, 10, 5)),cex.axis=2, las=2, tcl = -.8)
