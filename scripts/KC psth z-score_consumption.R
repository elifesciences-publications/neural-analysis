library(matrixStats)
load("C:/Users/Kevin Caref/Google Drive/RScripts/Functions/neuralhist.rFunc")

filepath = "C:/Users/Kevin Caref/Google Drive/Opioid Pilot/Neural Data/sorted/"
binw = 100
psthmin = 0
psthmax = 10
event = 6
cueexonly = F
startt = 0
endt = 7200
basebins = 50   #number of bins
baseline = seq(1,basebins)

sated = neuralhist.rFunc(path = paste(filepath, "sated response analysis/", sep = ""), startt,
 endt, binw, psthmin, psthmax, event, cueexonly)
 

restricted = neuralhist.rFunc(path = paste(filepath, "restricted response analysis/", sep = ""), startt=0,
 endt=7200, binw, psthmin, psthmax, event, cueexonly=F)

restricted = restricted[[1]][,restricted[[2]]]
sated = sated[[1]][,sated[[2]]]

#calculates the mean and standard deviation for each neuron
satedsds = sapply(seq(1, ncol(sated)), function(x) sd(as.numeric(sated[baseline,x])))
satedmeans = sapply(seq(1, ncol(sated)), function(x) mean(as.numeric(sated[baseline,x])))

#calculates a z-score for each bin for each neuron
satedzresp = as.matrix(sapply(seq(1, ncol(sated)), function(x)
  (as.numeric(sated[,x])-satedmeans[x])/satedsds[x]))
  
  

#calculates the mean and standard deviation for each neuron
restsds = sapply(seq(1, ncol(restricted)), function(x) sd(as.numeric(restricted[baseline,x])))
restmeans = sapply(seq(1, ncol(restricted)), function(x) mean(as.numeric(restricted[baseline,x])))

#calculates a z-score for each bin for each neuron
restzresp = as.matrix(sapply(seq(1, ncol(restricted)), function(x)
  (as.numeric(restricted[,x])-restmeans[x])/restsds[x]))

#detect any missing values for sessions with no trials
#torem = which(is.na(restzresp[1,]))


meansatedfiring = rowMeans(satedzresp)
meanrestrfiring = rowMeans(restzresp, na.rm = T)


satederror = rowSds(satedzresp)/sqrt(ncol(satedzresp))
restrerror = rowSds(restzresp, na.rm = T)/sqrt(ncol(restzresp))



xcoords = seq(-psthmin+(binw/1000)/2, psthmax-(binw/1000)/2, binw/1000)


par(pty = "s")
plot.new()
plot.window(xlim = c(-.5,.5), ylim = c(0, 20))
#rect(.1, -5, .3, 25, lwd =0, col = "gray84", border = "transparent")
lines(xcoords, meansatedfiring, col = "blue", lwd = 3)
lines(xcoords, meanrestrfiring, col= "firebrick1", lwd =3)

axis(1, at = c(seq(-.5, .5, .5)), cex.axis=2, tcl = -.8)
axis(2, at = c(seq(0, 20, 5)),cex.axis=2, las=2, tcl = -.8)
abline(v=0, col = "gray", lwd = 2)


#error cloud for pre firing
testcol=c(as.vector (col2rgb("blue",alpha=F)[,1]))
polcol=rgb(testcol[1],testcol[2],testcol[3],100, names = NULL, maxColorValue = 255)
polygon(c(xcoords,rev(xcoords)), c(meansatedfiring+satederror, rev(meansatedfiring-satederror)), density=-20, col= polcol,border=NA)

#error cloud for post firing
testcol=c(as.vector (col2rgb("firebrick1",alpha=F)[,1]))
polcol=rgb(testcol[1],testcol[2],testcol[3],100, names = NULL, maxColorValue = 255)
polygon(c(xcoords,rev(xcoords)), c(meanrestrfiring+restrerror, rev(meanrestrfiring-restrerror)), density=-20, col= polcol,,border=NA)

mtext("Time (s)", side = 1, line = 3, cex= 2)
mtext("Z-score", side = 2, line = 3, cex=2)
mtext("Sated", side = 1, line = -20, at = -.3, cex=1.75, col = "blue")
mtext("Restricted", side = 1, line = -18, at = -.3, cex=1.75, col="firebrick1")


#stats
pvals = sapply(which(xcoords >= 0 & xcoords <=.5), function(x) wilcox.test(x=as.numeric(satedzresp[x,]), y=as.numeric(restzresp[x,]), paired = F, correct = F)$p.value)
sigps = pvals[which(pvals <= .05)]
#text(xcoords[which(pvals <= .05)+length(which(xcoords<.1)) ], rep(30, length(sigps)), labels = "_", cex = 1.5)


cpvals = p.adjust(pvals, method = "bonf")

sigbarfrom = seq(-psthmin,psthmax-(binw/1000),binw/1000)[which(xcoords >= 0 & xcoords <=.3)]
sigbarto = seq(-psthmin+(binw/1000),psthmax,binw/1000)[which(xcoords >= 0 & xcoords <=.3)]  
 
segments(sigbarfrom[which(cpvals <= .05) ], rep(15, length(which(cpvals <=.05))), sigbarto[which(cpvals <= .05) ], rep(15, length(which(cpvals <=.05))), lwd =2,  cex = 1.5)



#sum(diff(xcoords[501:550]) * (head(meansatedfiring[501:550],-1)+tail(meansatedfiring[501:550],-1)))/2
#sum(diff(xcoords[501:550]) * (head(meanrestrfiring[501:550],-1)+tail(meanrestrfiring[501:550],-1)))/2





