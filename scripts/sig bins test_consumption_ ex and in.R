...
expresigbins = KC.sigbins_cons.Rfunc(path = paste(filepath, "sated response analysis/", sep = ""), startt, endt)
 
expostsigbins = KC.sigbins_cons.Rfunc(path = paste(filepath, "restricted response analysis/", sep = ""), startt=0, endt=7200)


excueexneuronspre = which(unlist(lapply(seq(1:ncol(expresigbins)), function(x) length(which(expresigbins[1:8,x] == T)))) != 0)
excueexneuronspost = which(unlist(lapply(seq(1:ncol(expostsigbins)), function(x) length(which(expostsigbins[1:8,x] == T)))) != 0)
...
exsigpost = expostsigbins[,which(unlist(lapply(seq(1:ncol(expostsigbins)), function(x) length(which(expostsigbins[,x] == T)))) != 0)]  #must look at same neurons
...
expostpct = sapply(seq(1,nrow(exsigpost)), function(x) {
  length(which(exsigpost[x,] == T))/ncol(expostsigbins)
  }) 
...
expresmooth = kcsmooth(exprepct, window = 3)
expostsmooth = kcsmooth(expostpct, window = 3)  

#find all inhibited bins
inpresigbins = KC.sigbins_inhib_cons.Rfunc(path = paste(filepath, "sated response analysis/", sep = ""), startt, endt)
 
inpostsigbins = KC.sigbins_inhib_cons.Rfunc(path = paste(filepath, "restricted response analysis/", sep = ""), startt=0, endt=7200)


incueexneuronspre = which(unlist(lapply(seq(1:ncol(inpresigbins)), function(x) length(which(inpresigbins[1:8,x] == T)))) != 0)
incueexneuronspost = which(unlist(lapply(seq(1:ncol(inpostsigbins)), function(x) length(which(inpostsigbins[1:8,x] == T)))) != 0)


#include only significantly excited neurons in the analysis
insigpre = inpresigbins[,which(unlist(lapply(seq(1:ncol(inpresigbins)), function(x) length(which(inpresigbins[,x] == T)))) != 0)]
insigpost =  inpostsigbins[,which(unlist(lapply(seq(1:ncol(inpostsigbins)), function(x) length(which(inpostsigbins[,x] == T)))) != 0)]  #must look at same neurons
#sigpost =  postsigbins[,which(unlist(lapply(seq(1:ncol(postsigbins)), function(x) length(which(postsigbins[,x] == T)))) != 0)]
...
inpostpct = sapply(seq(1,nrow(insigpost)), function(x) {
  length(which(insigpost[x,] == T))/ncol(inpostsigbins)
  }) 




inpresmooth = kcsmooth(inprepct, window = 3)
inpostsmooth = kcsmooth(inpostpct, window = 3) 
...
mtext(paste("Sated", sep = "-"), side = 1, line = -20, at = .4, cex=1.75, col = "blue")
mtext(paste("Restricted", sep = "-"), side = 1, line = -18, at = .4, cex=1.75, col="firebrick1")
...
par(pty = "s")
plot.new()
plot.window(xlim = c(0,2), ylim = c(-100,100))
...
axis(2, at = c(seq(-100, 100,50)), las = 2, labels = c(seq(100,0,-50), seq(50,100,50)), cex.axis = 1.75, tcl= -.8)

mtext("% Bins Excited", side = 2, line = 3.5, cex = 1.75, at = 50)
mtext("% Bins Inhibited", side = 2, line = 3.5, cex = 1.75, at = -50)

pex = wilcox.test(prepctexbins, postpctexbins, paired = F)$p.value
pin = wilcox.test(prepctinbins, postpctinbins, paired = F)$p.value
