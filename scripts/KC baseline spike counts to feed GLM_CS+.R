...
########################################################################
basemaster = lapply(seq(1, length(spikes)), function(x) {
 lapply(seq(1, length(events[[x]])), function(y) {
  basecount = length(which(spikes[[x]] >= events[[x]][y] - basewin & spikes[[x]] <= events[[x]][y]))
  #evokedcount = length(which(spikes[[x]] >= events[[x]][y] + winmin & spikes[[x]] <= events[[x]][y] + winmax))
  #normcount = evokedcount - basecount
  return(basecount)
   })
  })
#basemean = lapply(basemaster, function(x) (mean(unlist(x))/basewin))
#basesd = lapply(basemaster, function(x) sd(unlist(x)/basewin))   
#
#
#masterlist = lapply(seq(1, length(spikes)), function(x) {
# lapply(seq(1, length(events[[x]])), function(y) {
#  evokedcount = ((length(which(spikes[[x]] >= events[[x]][y] + winmin & spikes[[x]] <= events[[x]][y] + winmax))/basewin)-basemean[[x]])/basesd[[x]]
#    })
#  }) 
...
save(baseline_spikecounts.rFunc, file = "C:/Users/Kevin Caref/Google Drive/RScripts/Functions/baseline_spikecounts.rFunc") 
