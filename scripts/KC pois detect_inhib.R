KC.pois.detect_inhib.Rfunc = function(nexdata, startt, endt) {
...
cueinhib = lapply(seq(1, length(masterlist)), function(x) {
...
#Applying Vince's code to my data
    ...
    if(length(which(critwin <= .01)) > 0) (cueinhib = T)
...
save(KC.pois.detect_inhib.Rfunc, file = "C:/Users/Kevin Caref/Google Drive/RScripts/Functions/KC.pois.detect_inhib.Rfunc")
