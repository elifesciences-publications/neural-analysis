restricted = neuralhist.rFunc(path = "C:/Users/Kevin Caref/Google Drive/Opioid Pilot/Neural Data/sorted/restricted response analysis/", startt = 0,
 endt = 7200, binw = 20, psthmin = 1, psthmax = 1, event = 4, cueexonly = T)


neuralhist_inhib.rFunc = function(path, startt, endt, binw, psthmin, psthmax, event, cueinhibonly) { 
...
#########################################################################
if(cueinhibonly == T) (cueinhibidx = KC.pois.detect_inhib.Rfunc(nexdata, startt, endt))
...
if(cueinhibonly == T) ({
  cueinhibneurons = which(unlist(cueinhibidx) == T)
   })
   
if(cueinhibonly == T) (return(list(firingmat,cueinhibneurons)))
if(cueinhibonly == F) (return(firingmat))
}

save(neuralhist_inhib.rFunc, file = "C:/Users/Kevin Caref/Google Drive/RScripts/Functions/neuralhist_inhib.rFunc") 
