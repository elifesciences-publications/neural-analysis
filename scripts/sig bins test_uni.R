...
presigbins = KC.sigbins_uni.Rfunc(path = paste(filepath, "unilateral ctap/", sep = ""), startt, endt, event, side)
 
postsigbins = KC.sigbins_uni.Rfunc(path = paste(filepath, "unilateral ctap/", sep = ""), startt=2720, endt=4720, event, side)
...
postpct = sapply(seq(1,nrow(sigpost)), function(x) {
  length(which(sigpost[x,] == T))/ncol(presigbins)
  }) 
...
par(pty = "s")  
plot.new()
plot.window(xlim = c(0,.5), ylim = c(0, 100))
...
axis(2, at = seq(0,100,20), cex.axis = 2, las = 2)
...
axis(2, at = c(seq(0, 100,25)), las = 2, cex.axis = 1.75, tcl= -.8)
